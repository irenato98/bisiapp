...
check(wt.includes("type: 'both-changed'") && wt.includes("reason, ids, conflicts"), 'same-activity concurrent changes become explicit conflict review');
...
check(wt.includes("Number(error?.status || 0) === 409") && wt.includes("'backend-version-conflict'"), 'backend 409 becomes stale-write review rather than overwrite');
...
check(wt.includes('function acceptRemoteConflicts') && wt.includes("reason: 'remote-conflicts-accepted'"), 'explicit remote-wins conflict resolver exists');
check(wt.includes('remote-only addition is never') === false, 'source contains no test-only prose');
check(readme.includes('Connection 8') && readme.includes('optimistic concurrency'), 'README documents Connection 8 concurrency contract');
