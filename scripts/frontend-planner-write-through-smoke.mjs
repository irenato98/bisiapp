...
check(writeThroughSection.includes("op?.deleteIntent === 'structural'") && writeThroughSection.includes('op?.structuralDeleteIds'), 'recurrence pruning records only explicit structural delete ids');
...
check(writeThroughSection.includes('const verify = await window.BisiBackendConnection.listTasks();') && writeThroughSection.includes('applyVerifiedSnapshotWhenSafe(remote)'), 'write batch re-reads fresh backend snapshot and applies it only when interaction-safe');
check(writeThroughSection.includes('deferredVerifiedSnapshot') && writeThroughSection.includes('plannerInteractionBlocksRefresh()'), 'own verified hydration defers during editor, Focus or drag');
